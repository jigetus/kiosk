...
function AddKruzhok(id, name, img, author, desc, age, time, price) {
    ...
    tab.innerHTML = '<div class="tabs noselect faster kruzhki_info" id="tab' + id + '">    <div class="tab_title">  <button class="back_action">      <img src="/images/close.svg" alt="" class="back_btn noselect" /> </button>        <div>        </div>    </div>    <div class="kruzhki_info_container">        <img src="/images/kruzhki/' + img + '"></img>        <h1>' + name + '</h1>        <span>' + desc + '</span>        <span class="age">' + age + '</span>        <span class="time">' + time + '</span>        <span class="price">' + price + '</span>        <span class="teacher">Преподаватель: ' + author + '</span>        <img src="/images/kruzhki/qr.jpg" alt="" class="qr">    </div></div>';
    ...
    fetch(window.location.href + '/data/kruzhki.json')
        ...
        .then(array => {
            array.forEach(element => {
                AddKruzhok(element.id, element.name, element.img, element.author, element.desc, element.age, element.time, element.price);
            });
            //add scrollbars
